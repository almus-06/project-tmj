                            ...
                            <tr class="hover:bg-indigo-50 transition-colors cursor-pointer group border-b border-slate-200/60" style="{{ $loop->even ? 'background-color: #EFEFEF;' : '' }}" @click="expanded = expanded === {{ $row->id }} ? null : {{ $row->id }}">
                                <td class="px-5 py-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-black text-slate-900 text-xs">{{ $row->unit->no_kendaraan ?? '—' }}</p>
                                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{{ $row->unit->jenis_alat ?? '—' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-2">
                                    @if($row->status === 'Ready')
                                        <span class="status-chip badge-ready" style="padding-top: 2px; padding-bottom: 2px;">Ready</span>
                                    @elseif($row->status === 'Standby')
                                        <span class="status-chip badge-standby" style="padding-top: 2px; padding-bottom: 2px;">Standby</span>
                                    @else
                                        <span class="status-chip badge-down" style="padding-top: 2px; padding-bottom: 2px;">Down</span>
                                        @if($row->damage_type)
                                            <p class="text-[9px] text-red-500 font-bold mt-1 uppercase tracking-tighter">{{ Str::limit($row->damage_type, 30) }}</p>
                                        @endif
                                    @endif
                                </td>
                                <td class="px-5 py-2">
                                    <p class="text-xs text-slate-700 font-bold">{{ $row->operator->name ?? '—' }}</p>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">{{ $row->project->name ?? '—' }} • {{ $row->location }}</p>
                                </td>
                                <td class="px-5 py-2 text-right tabular-nums">
                                    ...
                                    @if($isHM)
                                        <p class="text-xs font-black text-slate-800">{{ number_format($row->hm, 1) }} <span class="text-[9px] text-slate-400 font-bold">HM</span></p>
                                    @else
                                        <p class="text-xs font-black text-slate-800">{{ number_format($row->km, 1) }} <span class="text-[9px] text-slate-400 font-bold">KM</span></p>
                                    @endif
                                </td>
                            </tr>
