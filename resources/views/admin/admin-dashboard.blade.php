...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-sm text-slate-500 font-medium mt-1">PT. TRI MACHMUD JAYA - {{ now()->format('d M Y') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 {{ $statCols }} gap-5 mb-8">
        ...
        @if(in_array(Auth::user()->role, ['admin', 'supervisor', 'workshop']))
            {{-- Unit Ready --}}
            <div
                class="card-industrial p-6 flex flex-col items-center text-center border-b-4 border-b-green-500 overflow-hidden relative group">
                ...
                <p class="text-[10px] font-black uppercase tracking-widest text-green-600 mb-1 relative z-10">Unit Siap</p>
            ...
            <div class="card-industrial p-6 flex flex-col items-center text-center border-b-4 border-b-red-500">
                ...
                <p class="text-[10px] text-red-400 font-bold mt-2 uppercase">Unit Tidak Beroperasi</p>
            </div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 {{ $navCols }} gap-4 sm:gap-5">
        @if(in_array(Auth::user()->role, ['admin', 'supervisor', 'hrd']))
            <a href="{{ route('workforce.attendance') }}"
                ...
                <div
                    class="absolute right-0 bottom-0 p-4 opacity-5 translate-y-4 translate-x-4 group-hover:translate-y-0 group-hover:translate-x-0 transition-transform hidden sm:block">
                ...
                <div
                    class="w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center bg-indigo-950 text-white flex-shrink-0 transition-transform group-hover:scale-110">
                ...
                <div
                    class="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-slate-100 flex items-center justify-center group-hover:bg-indigo-500 group-hover:text-white transition-colors relative z-10">
        ...
        @if(in_array(Auth::user()->role, ['admin', 'supervisor', 'workshop']))
            <a href="{{ route('fleet.management') }}"
                ...
                <div
                    ...
                    <svg class="w-32 h-32 text-slate-900" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l9 1m1-11h4l3 5v4h-7V5z" />
                    </svg>
                </div>
                <div
                    class="w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center bg-indigo-950 text-white flex-shrink-0 transition-transform group-hover:scale-110">
    ...
    <script>
        // Use a function to allow re-init by Swup or immediate run
        (function() {
            console.log('Dashboard refresh timer started');
            setTimeout(() => {
                if (window.location.pathname === '{{ route('operations.dashboard', [], false) }}') {
